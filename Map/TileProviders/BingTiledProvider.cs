...
namespace Ptv.XServer.Controls.Map.TileProviders
{
    ...
    public class BingMetaInfo
    {
        ...
        {
            Key = key;
            var url = $@"https://dev.virtualearth.net/REST/v1/Imagery/Metadata/{Enum.GetName(typeof(BingImagerySet), imagerySet)}?mapVersion={Enum.GetName(typeof(BingMapVersion), mapVersion)}&uriScheme=https&o=xml&key={key}";
